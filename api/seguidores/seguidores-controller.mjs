...
const { seguidores } = initModels(dbSequelize);

export const seguidoresController = {
    post: async(req, res) => {
        try {
            const data = await seguidores.create({
                ...req.body,
            });
            res.json(data);
        } catch (error) {
    ...
    get: async(req, res) => {
        try {
            const data = await seguidores.findByPk(req.params.id);
            res.json(data);
        } catch (error) {
            console.log(error);
            return res.status(500).json(error);
    ...
    gets: async(req, res) => {
        try {
            ...
            const {count:total, rows: data} = await seguidores.findAndCountAll({
    ...
    delete: async(req, res) => {
        try {
            const data = await seguidores.destroy({
