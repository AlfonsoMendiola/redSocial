...
const { usuarios, rolesUsuarios, roles } = initModels(dbSequelize);

export const usuariosController = {
    postUsuario: async(req, res) => {
        try {
            await dbSequelize.query("START TRANSACTION");
            ...
            const rol = await roles.findOne({ where: { nombre: 'miPerfil' } });
            await rolesUsuarios.create({
                id_usuario: usuario.id,
                id_rol: rol.id
            });

            await dbSequelize.query("COMMIT");
            const data = await usuarios.findByPk(usuario.id, { attributes: { exclude:['pass', 'tipoUsuario'] } });
            res.json(data);
        } catch (error) {
            await dbSequelize.query("ROLLBACK");
            console.log(error);
