...
const { redesUsuario } = initModels(dbSequelize);

export const redesUsuarioController = {
    post: async(req, res) => {
        try {
            const data = await redesUsuario.create({
                ...req.body,
            });
            res.json(data);
        } catch (error) {
    ...
    get: async(req, res) => {
        try {
            const data = await redesUsuario.findByPk(req.params.id);
            res.json(data);
        } catch (error) {
            console.log(error);
            return res.status(500).json(error);
    ...
    gets: async(req, res) => {
        try {
            ...
            const {count:total, rows: data} = await redesUsuario.findAndCountAll({
    ...
    delete: async(req, res) => {
        try {
            const data = await redesUsuario.destroy({
